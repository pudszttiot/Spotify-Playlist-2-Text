import sys
import re
from PyQt5.QtWidgets import (QApplication, QWidget, QLabel, QLineEdit, QPushButton, QMessageBox, QFileDialog, QVBoxLayout, QProgressBar, QHBoxLayout, QComboBox)
...
class SpotifyPlaylistApp(QWidget):
    ...
    def start_playlist_processing(self):
        ...
        file_types = {"Text (.txt)": "Text files (*.txt)", "HTML (.html)": "HTML files (*.html)", "CSV (.csv)": "CSV files (*.csv)"}
        output_file, _ = QFileDialog.getSaveFileName(self, "Save File", "", file_types[export_format])
    ...
    def validate_spotify_url(self, url):
        ...
        spotify_url_pattern = re.compile(r"https?://open\.spotify\.com/playlist/[A-Za-z0-9]+")
...
class PlaylistWorker(QThread):
    ...
    def run(self):
        ...
        if tracks:
            ...
            if self.export_format == "Text (.txt)":
                success = self.save_tracks_as_text(tracks, self.output_file, self.playlist_url, playlist_name, total_tracks)
            elif self.export_format == "HTML (.html)":
                success = self.save_tracks_as_html(tracks, self.output_file, self.playlist_url, playlist_name)
            elif self.export_format == "CSV (.csv)":
                success = self.save_tracks_as_csv(tracks, self.output_file, playlist_name)
    ...
    def save_tracks_as_text(self, tracks, filename, playlist_url, playlist_name, total_tracks):
        ...
        try:
            with open(filename, 'w', encoding='utf-8') as file:
                ...
                for i, track in enumerate(tracks):
                    file.write(f"{track['artists']} - {track['track_name']} (Spotify URL: {track['track_url']})\n")
                ...
                <body>
                    <div class="container">
                        <div class="playlist-header">
                            <h1>{playlist_name}</h1>
                            <p><a href="{playlist_url}">Open Playlist on Spotify</a></p>
                        </div>
                        <ul class="track-list">
                """)

                # Add each track to the HTML file with improved layout and hover effect
                for track in tracks:
                    file.write(f"""
                        <li class="track-item">
                            <div class="track-details"><strong>{track['artists']}</strong> - {track['track_name']}</div>
                            <div class="track-url"><a href="{track['track_url']}">Listen on Spotify</a></div>
                        </li>
                    """)
    ...
    def save_tracks_as_csv(self, tracks, filename, playlist_name):
        """Save track details as a CSV file."""
        try:
            with open(filename, 'w', newline='', encoding='utf-8') as file:
                writer = csv.writer(file)
                writer.writerow(['Playlist Name', 'Artist', 'Track Name', 'Spotify URL'])
                writer.writerow([playlist_name, '', '', ''])  # Playlist name row
                
                for track in tracks:
                    writer.writerow([playlist_name, track['artists'], track['track_name'], track['track_url']])
